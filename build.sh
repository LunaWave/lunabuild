...
ifargs=false
logs=(
    "Build ID: lb_build_id:$(date +"%Y%m%d_%H%M%S");$os_number:$os;project_name:($(basename "$(pwd)"))"
    ">LOG<"
)
getonlinedata() {
...
resetproject() {
  wrn "Resetting project."
            
  [ -d ./build ] && rm -r ./build
  [ -d ./buildversions ] && rm -r ./buildversions
  [ -d ./temp ] && rm -r ./temp
...
log " "
# Anything before this is logged
# Main Sequence
log " - > - > MAIN SEQUENCE < - < -"
...
print "\t     Powered by Bash ♥     " "cyan" "*" "blink"
divd
if [ -e "./buildversions/versions" ]; then
...
for item in "./buildversions/versions"/*/; do
...
mkdir -p ./buildversions/recent
rm -r ./buildversions/recent
mkdir ./buildversions/recent
mkdir -p ./buildversions/versions
cp -r ./build/ ./buildversions/recent/build/
cp -r ./temp/"$log_file" ./buildversions/recent
cp -r ./buildversions/recent ./buildversions/versions/"$highest_number"/
